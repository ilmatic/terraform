...
func resourceAwsSpotInstanceRequestCreate(d *schema.ResourceData, meta interface{}) error {
	...
	// In Terraform, we always set the type to "persistent", since "one-time" is
	// for more of an imperative use case, while "persistent" is declarative,
	// which fits in TF's domain.
	spotType := "persistent"
